...
class WingAndPowerSizing:
    ...
    def __init__(self):
        ...
        self.CL_CD_cruise = self.CLmax_clean / (
                    self.CD0_clean + (self.CLmax_clean ** 2 / (self.Oswald_clean * self.AR * np.pi)))
    ...
    def climbgradient(self, x):
        y = self.n_p / (
                    np.sqrt(x) * (self.c_V + self.CL_CD_TO ** -1) * np.sqrt(2 / (ISA_density * (self.CLmax_TO - 0.2))))
        return y
    ...
    def plot_power(self, landing, cruise):
        ...
        if cruise:
            ...
            plt.plot(x_list, self.climbgradient(x_list), linestyle="solid", color="red",
                     label="Climb gradient constraint")
