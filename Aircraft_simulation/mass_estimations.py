...
class Aircraft(WingAndPowerSizing):
    ...
    def class1(self):
        ...
        if self.iter > 0:
            ...
            self.w_mtow = (self.w_payload + self.b + self.w_crew) / (
                        1 - self.a - self.fuel_factor * (1 + self.f_res) * 40 / 120)
            self.w_fuel = self.fuel_factor * (1 + self.f_res) * self.w_mtow * 40 / 120
            self.w_oew = self.w_mtow - self.w_fuel - self.w_payload
            self.surface_wing = self.w_mtow / self.w_s
            self.b_w = np.sqrt(self.AR * self.surface_wing)
    ...
    def powertrain_mass(self):
        ...
        self.w_installedEngine = 1.2 * (
                    self.m_electric_engine + self.m_fuel_cell + self.m_pmad + self.m_cooling + self.m_comp)
    ...
    def reset(self):
        ...
        self.insulation_length = 2.1 * self.meters_to_feet
        self.length_fus = [
            (self.cockpitlength + self.payloadlength + self.length_tailcone + 0.22 * self.meters_to_feet)]
    ...
    def class2(self):
        ...
        self.m_fuselage.append(0.052 * (self.length_fus[-1] * self.diameter_fus * np.pi) ** 1.086 * (
                    self.w_design * self.limit_load * self.limit_factor) ** 0.117 * self.lh ** (-0.051) * (
                                   self.CL_CD_cruise) ** (-0.072) * self.q ** 0.241 + 11.9 + (
                                           self.pressurised_volume * 8) ** 0.271)
        self.m_wing.append(0.036 * self.surface_wing ** (0.758) * (1300 * 2.208) ** 0.0035 * (
                    self.AR / (np.cos(self.sweep_angle) ** 2)) ** 0.6 * self.q ** (0.006) * self.taper_ratio ** 0.04 * (
                                       100 * self.t_c / (np.cos(self.sweep_angle))) ** -0.3 * (
                                       self.limit_factor * self.limit_load * self.w_design) ** 0.49)
        # Horizontal stabilizer mass
        self.m_h = 0.016 * (self.limit_factor * self.limit_load * self.w_design) ** 0.414 * self.q ** (
            0.168) * self.surface_controlh ** (0.896) * \
                   (100 * self.t_c / np.cos(self.sweep_angle_horizontal)) ** (-0.12)

        # Vertical stabilizer mass
        self.m_v = 0.073 * (1 + 0.2 * (Ht_Hv)) * (
                    1.5 * 2.5 * self.w_design) ** 0.376 * self.q ** 0.122 * self.surface_controlv ** (0.873) * \
                   (100 * self.t_c / (np.cos(self.sweep_angle_vertical))) ** (-0.49) * \
                   (self.AR / (np.cos(self.sweep_angle_vertical) ** 2)) ** (0.357) * (
                       self.taper_ratiov) ** 0.039  # checked
        ...
        self.w_flightcontrols = 0.053 * self.length_fus[-1] ** (1.536) * self.b_w ** (0.371) * (
                    self.limit_factor * self.limit_load * self.w_design * 10 ** -4) ** 0.8
    ...
    def plot_mass_progression(self):
        labels = ['fuselage', 'horizontal stab', 'vertical stab', 'wing', 'furnishing', ' de-icing', ' electronics',
                  'avionics', ' fuelsystem', ' flightcontrols', 'engine', ' hydraulics']
    ...
    def cgcalc(self):
        ...
        self.x_wing_cg = ((self.tip_chord ** 2 * 0.5 * self.b_w) + (
                    (self.root_chord - self.tip_chord) * 0.5 * self.b_w) * (
                                      self.tip_chord + (2 / 3) * (self.root_chord - self.tip_chord))) / (
                                     self.b_w * self.tip_chord + (
                                         self.root_chord - self.tip_chord) * self.b_w * 0.5) / self.root_chord
        self.y_mac = ((self.tip_chord * self.b_w * 0.5) + ((self.root_chord - self.tip_chord) * self.b_w * 0.5) * (
                    1 / 3)) / (((self.root_chord - self.tip_chord) * self.b_w * 0.5) + self.tip_chord * self.b_w)

    def cg_lists(self):
        weights = {"fuselage": self.m_fuselage[-1], "empennage": self.m_h + self.m_v, "mlg": self.m_mlg,
                   "nlg": self.m_nlg, "crew": self.w_crew, "wing": self.m_wing[-1], "battery": self.w_battery,
                   "engine": self.w_installedEngine, "fueltank": self.w_fueltank, "mtow": self.w_mtow,
                   "oew": self.w_oew, "payload": self.w_payload, "fuel": self.w_fuel}
        fuselage_cg = {"fuselage": self.x_fuselage_cg, "empennage": self.x_empennage_cg, "mlg": self.x_landingGear_cg,
                       "nlg": self.x_nlg_cg, "crew": self.x_fuselage_cg, "fueltank": self.x_fuel_cg,
                       "payload": self.x_payload_cg, "fuel": self.x_fuel_cg}
    ...
    def plot_mass_breakdown(self):
        ...
        for idx, component in enumerate(ac_component_masses[0]):
            perc = int(100 * component / self.w_mtow)
